...
public abstract class Assembly implements Enumeration<Object>, PubliclyCloneable {
	 ...
	 * recognising an HTML page, for example, it might create a Page object early,
	 * and store it as an assembly's "target". As its recognition of the HTML
	 * progresses, it could use the stack to build intermediate results, like a
	 * heading, and then apply them to the target object.
	 */
	...
	/**
	 * Returns the elements of the assembly that have been consumed, separated by
	...
	/**
	 * Returns the object identified as this assembly's "target". Clients can set
	 * and retrieve a target, which can be a convenient supplement as a place to
	 * work, in addition to the assembly's stack. For example, a parser for an HTML
	 * file might use a web page object as its "target". As the parser recognizes
	 * markup commands like <head>, it could apply its findings to the target.
	 * 
	...
	/**
	 * Returns the elements of the assembly that remain to be consumed, separated by
	 * the specified delimiter.
	 *
	 * @param String the mark to show between unconsumed elements
	 *
