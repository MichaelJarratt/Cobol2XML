...
public abstract class Parser {
	 ...
	 *
	 * @param String A name to be known by. For parsers that are deep composites, a
	 ...
	 * The book, "Design Patterns", explains the visitor pattern.
	 *
	 ...
	 *
	 * @return an assembly with the greatest possible number of elements consumed by
	 *         this parser
	 *
	...
	/**
	 * Given a set (well, a <code>Vector</code>, really) of assemblies, this method
	 * matches this parser against all of them, and returns a new set (also really a
	 * <code>Vector</code>) of the assemblies that result from the matches.
	 * <p>
	 * For example, consider matching the regular expression <code>a*</code> against
	 * the string <code>"aaab"</code>. The initial set of states is
	 * <code>{^aaab}</code>, where the ^ indicates how far along the assembly is.
	 * When <code>a*</code> matches against this initial state, it creates a new set
	 * <code>{^aaab, a^aab, aa^ab, 
	 * aaa^b}</code>.
	 * 
	 * @return a Vector of assemblies that result from matching against a beginning
	 *         set of assemblies
	 *
	...
	/**
	 * Returns a textual description of this parser. Parsers can be recursive, so
	 * when building a descriptive string, it is important to avoid infinite
	 * recursion by keeping track of the objects already described. This method
	 * keeps an object from printing twice, and uses <code>unvisitedString</code>
